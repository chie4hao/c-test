...
namespace wzd {
	namespace cpptest {
		...
		void FrameUtil::SequenceScramble(std::shared_ptr<uint8_t> sequence_buffer_ptr_, uint64_t pos) const {
			auto seq = sequence_buffer_ptr_.get();
			auto frame_array = frame_buffer_.get();
		...
		void FrameUtil::ExtractRow(uint64_t begin /*bit*/, size_t frame_length /*byte*/) {
			auto frame_array = frame_buffer_.get();
			size_t frame_count = frame_count_ - static_cast<size_t>(begin) / 8 / frame_length_;
			int8_t offset = begin % 8;

			for (uint64_t i = 0; i < frame_count*frame_length; ++i) {
				frame_array[i] = (frame_array[i / frame_length*frame_length_ + i%frame_length + begin / 8] << offset)
					+ (frame_array[i / frame_length*frame_length_ + i%frame_length + begin / 8 + 1] >> (8 - offset));
			}
		...
		void FrameUtil::Matrix(const int32_t a, const int32_t b)
		{
			...
			for (size_t frame = 0; frame < frame_count_*frame_length_ / matrix_length; ++frame) {
		...
		void FrameUtil::golayDecode() {
			...
			uint16_t golay_array[23]{
				0x0001, 0x0002, 0x0004, 0x0008,
				0x0010, 0x0020, 0x0040, 0x0080,
				0x0100, 0x0200, 0x0400, 0x0475,
				0x049F, 0x054B, 0x06E3, 0x01B3,
				0x0366, 0x06CC, 0x01ED, 0x03DA,
				0x07B4, 0x031D, 0x063A
			};
			for (int i = 0; i < 21; ++i) {
				for (int j = i + 1; j < 22; ++j) {
					for (int k = j + 1; k < 23; ++k) {
						golay_map[golay_array[i] ^ golay_array[j] ^ golay_array[k]] = (1 << i) + (1 << j) + (1 << k);
					}
				}
			}
			...
			while (length-- > 0) {
				...
				if (isOffset) {
					*(write_ptr + 1) = (*(write_ptr + 1) & 0xF0) + (((*frame_array ^ (golay_code >> 15)) >> 4) & 0x0F);
					*(write_ptr + 2) = (((*frame_array ^ (golay_code >> 15)) << 4) & 0xF0) + (((*(frame_array + 1) ^ (golay_code >> 7)) >> 4) & 0x0F);
					write_ptr += 3;
				}
				else {
